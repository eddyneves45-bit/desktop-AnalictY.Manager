...
$publishedExe = Join-Path $resolvedPublishRoot "AnalictY.Console.exe"
...
Get-Process -Name "AnalictY.Console" -ErrorAction SilentlyContinue |
...
if (Test-Path $publishRoot) {
    ...
    for ($attempt = 1; $attempt -le 5; $attempt++) {
        try {
            ...
            if ($attempt -eq 5) {
                throw "Nao foi possivel limpar a pasta de publicacao. Feche o AnalictY.Console aberto em release\desktop-console e tente novamente. Detalhes: $($_.Exception.Message)"
            }
